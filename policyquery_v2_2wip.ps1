...
foreach ($FinOpsAssignment in $FinOpsAssignments) {
    if (-not $individualAssignments.ContainsKey($FinOpsAssignment.PolicyAssignmentId)) {
        $individualAssignments[$FinOpsAssignment.PolicyAssignmentId] = $FinOpsAssignment
    }
}
$FHash = @{}
$FHash.FinOpsAssignments = @{}
foreach ($FinOpsAssignment in $FinOpsAssignments) {
    if ($FHash.FinOpsAssignments.keys -notcontains $FinOpsAssignment.PolicyAssignmentId) {
        $FHash.FinOpsAssignments.($FinOpsAssignment.PolicyAssignmentId) = @{}
        $FHash.FinOpsAssignments.($FinOpsAssignment.PolicyAssignmentId).Raw = $FinOpsAssignment 
        }
}
$testobj = @()
foreach ($entry in $FHash.FinOpsAssignments.keys) {
    $testobj += $entry
}
